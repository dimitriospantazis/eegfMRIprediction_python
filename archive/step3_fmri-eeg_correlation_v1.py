...
# ---------- Parameters ----------
fmri_index = 0  # fMRI time course index (choose which one to correlate)
network_channels = {  # Example: indices for networks
    'DAN': [0, 1, 2],   # replace with actual indices for DAN channels
    'DMN': [3, 4, 5]    # replace with actual indices for DMN channels
}
lags = np.linspace(0, 8, n_bins)      # Lag axis (s)
freqs = np.linspace(0, 40, n_freqs)   # Frequency axis (Hz)

# ---------- Function: compute correlation map ----------
def compute_corr_map(X_data, Y_data, channel_indices):
    fmri_signal = Y_data[:, fmri_index]
    eeg_features = np.mean(X_data[:, channel_indices, :, :], axis=1)  # average across channels in network
...
# ---------- Compute correlation maps for networks ----------
corr_DAN = compute_corr_map(X_normalized, Y, network_channels['DAN'])
corr_DMN = compute_corr_map(X_normalized, Y, network_channels['DMN'])

# ---------- Plot ----------
fig, axes = plt.subplots(1, 2, figsize=(18, 5), sharey=True)
vmin = -np.max(np.abs([corr_DAN, corr_DMN]))
vmax = np.max(np.abs([corr_DAN, corr_DMN]))
...
axes[0].set_title('DAN', fontsize=14)
...
axes[1].set_title('DMN', fontsize=14)
