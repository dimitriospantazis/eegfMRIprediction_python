...
N_PERM_NULL = 100         # e.g., 100–500 for paper; 0 for speed
PERM_SEED   = 1337
...
# =========================
# Per-subject normalization (as before) + progress bar
# =========================
...
# =========================
# Main loop (no folds; fit on ALL data) + progress bars
# =========================
...
for target_key in outer_iter:
    ...
    if N_PERM_NULL > 0:
        _tqdm_write(f"  Permutation null for |SHAP|: {N_PERM_NULL} permutations")
        null_means_abs_bt = np.zeros((N_PERM_NULL, n_bands, n_bins), dtype=np.float32)
        perm_pbar = tqdm(total=N_PERM_NULL, desc=f"  Perms {target_key}", unit="perm") if _TQDM else None
        ...
        _tqdm_write("  Permutation p-values computed.")
